...
const BottomNav = () => {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-[#262626] p-2 h-20 flex justify-around z-40 md:hidden">
      <button
        onClick={() => navigate("/")}
      ...
      <button
        ...
        className="absolute bottom-6 bg-red-500 text-white rounded-full p-4 items-center hover:bg-red-600 transition-colors"
      >
        <BiSolidDish size={40} />
      </button>
      ...
      <Modal isOpen={isModalOpen} onClose={closeModal} title="Create Order">
        ...
        <div>
          ...
          <div className="flex items-center justify-between bg-[#1f1f1f] px-4 py-3 rounded-lg">
            <button onClick={decrement} className="text-red-500 text-2xl">&minus;</button>
            <span className="text-white">{guestCount} Person</span>
            <button onClick={increment} className="text-red-500 text-2xl">&#43;</button>
          </div>
        </div>
        <button onClick={handleCreateOrder} className="w-full bg-red-500 text-white rounded-lg py-3 mt-8 hover:bg-red-600 transition-colors">
          Create Order
        </button>
      </Modal>
    </div>
